...
class ArtsPage extends Component {
  ...
  fetchData() {
    axios.get(`/api/subtopics/${8}`).then((response) => {
            ...
            <div id="inputFieldsCommentAndResource">
              ...
              {this.state.results.map((subtopics, i) => (
                <li key={i}>
                  ...
                  <Button id="convoTopic" variant="raised" onClick={this.sendUserToCorrespondingPage(`/arts_convo/${subtopics.id}`)}>{subtopics.subtopic}</Button>
                </li>
              ))}
            </ul>
          </div>
...
export default connect(mapStateToProps)(ArtsPage);
