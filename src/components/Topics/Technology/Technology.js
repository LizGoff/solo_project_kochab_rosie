...
class TechPage extends Component {
  ...
  fetchData() {
    axios.get(`/api/subtopics/${9}`).then((response) => {
            ...
            <div id="inputFieldsCommentAndResource">
              ...
              {this.state.results.map((subtopics, i) => (
                <li key={i}>
                  ...
                  <Button id="convoTopic" variant="raised" onClick={this.sendUserToCorrespondingPage(`/technology_convo/${subtopics.id}`)}>{subtopics.subtopic}</Button>
                </li>
              ))}
            </ul>
          </div>
