...
class SciencePage extends Component {
  ...
  fetchData() {
    axios.get(`/api/subtopics/${10}`).then((response) => {
            ...
            <ul id="shareTopicsButtons">
              {this.state.results.map((subtopics, i) => (
                <li key={i}>
                  ...
                  <Button id="convoTopic" variant="raised" onClick={this.sendUserToCorrespondingPage(`/science_convo/${subtopics.id}`)}>{subtopics.subtopic}</Button>
                </li>
              ))}
            </ul>
          </div>
