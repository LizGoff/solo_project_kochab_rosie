...
class WageGapPage extends Component {
  ...
  fetchData() {
    axios.get(`/api/subtopics/${25}`).then((response) => {
            ...
            <ul id="shareTopicsButtons">
              {this.state.results.map((subtopics, i) => (
                <li key={i}>
                  ...
                  <Button id="convoTopic" variant="raised" onClick={this.sendUserToCorrespondingPage(`/wage_gap_convo/${subtopics.id}`)}>{subtopics.subtopic}</Button>
                </li>
              ))}
            </ul>
          </div>
